...
const ProjectListItem = ({ projectItem, projectActive }) => {
	...
	let projectBackground = {
		'backgroundColor': projectItem.color[0]
	};
 	let projectGradient = {
 	 	background: 'linear-gradient(to bottom, '+ projectItem.color[0] +' 0%,'+ projectItem.color[1] +' 60%)'
 	};
	...
	return (		 	 
 		<div className={ projectClass } style={ projectBackground } >
			...
			<div className="projects-item-details">
 				...
 				<div className="projects-item-image browser-mockup" style={ projectGradient }>
			...
			<div className="projects-item-bottom">
				<div className="inner">
					...
					<Link to="/case/xivdps" className="projects-item-view icon linknew rect">Visit website</Link> 
				</div> 
			</div>
		</div>
	);
